#! /bin/bash
# Initial setup script for Debian 12 on Intel NUC6CAYH systems
#
# This script automates essential post-installation tasks for a clean Debian 12 setup,
...
echo "#deb cdrom:[Debian GNU/Linux 12.1.0 _Bookworm_ - Official amd64 NETINST with firmware 20230722-10:48]/ bookworm main non-free-firmware" >> /etc/apt/sources.list
...
echo "" >> /etc/apt/sources.list
echo "# bookworm-updates, to get updates before a point release is made;" >> /etc/apt/sources.list
...
echo "" >> /etc/apt/sources.list
echo "# This file was updated by a scrip to configure Debian 12 on Intel Nuc" >> /etc/apt/sources.list
echo "" >> /etc/apt/sources.list
echo "" >> /etc/apt/sources.list
echo "" >> /etc/apt/sources.list
